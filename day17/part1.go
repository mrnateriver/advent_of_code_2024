...
func RunProgram() string {
	...
	// 		fmt.Printf("%s%s\n", shared.Colored(shared.ColorRed, "Panic!"), strings.Repeat(" ", 32))
	// 		debugPrint(&cpu)
	// 		panic(err)
	// 	}
	// }()
...
func intPow(base, exp int) int {
	...
	if exp < 0 {
		panic(fmt.Errorf("cannot raise integer to fractional power with integer result; base: %v exp: %v", base, exp))
	}
